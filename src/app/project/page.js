...
export default function ProjectsPage() {
  ...
  return (
    <div className="space-y-5 max-w-7xl bg-[#7d3431] py-1 mx-auto px-1">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white py-4 px-5 rounded-xl shadow-sm border border-red-200/50">
        <div>
          <h2 className="text-lg font-bold text-black">Projects Directory</h2>
          <p className="text-xs text-black/70">Manage and track all your projects.</p>
        </div>
        <button 
              ...
              {currentProjects.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center py-16 text-black/40">
                    <FolderKanban className="w-10 h-10 mx-auto opacity-20 mb-2" />
                ...
                currentProjects.map((project) => (
                  <tr key={project.id} className="hover:bg-[#7d3431]/5 transition-colors">
                    ...
                    <td className="px-6 py-3">
                      <span className="font-semibold text-black block max-w-xs truncate">{project.title}</span>
                    </td>
        ...
        {filteredProjects.length > 0 && (
          <div className="px-6 py-3.5 border-t border-red-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-[#7d3431]/5">
            <span className="font-medium text-black/70 text-sm">
              ...
              of <span className="text-black font-semibold">{filteredProjects.length}</span> entries
            </span>
          ...
          <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-xl border border-red-200">
            ...
            <form onSubmit={editingId ? handleUpdateProject : handleCreateProject} className="space-y-4 text-sm">
              ...
              <div className="flex gap-2.5 pt-2">
                ...
                >
                  {editingId ? 'Update Project' : 'Save Changes'}
                </button>
              </div>
            </form>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm font-semibold text-black">{viewingProject.title}</p>
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-black">Description</p>
                  <p className="text-sm text-black/80">{viewingProject.description}</p>
                </div>
                <div className="flex gap-4">
                  <div className="flex-1">
                    <p className="text-xs font-bold uppercase tracking-wider text-black">Status</p>
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border mt-1 ${getStatusColor(viewingProject.status)}`}>
                      <span className={`w-1.5 h-1.5 rounded-full mr-1.5 
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-medium text-black">{viewingProject.createdAt}</p>
                  </div>
                </div>
              </div>
